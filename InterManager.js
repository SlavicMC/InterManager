...
client.on('ready', async () => {
    ...
    {
      guild.invites.fetch().then(async function(invites)
      {
        let uses = []
...
client.on('channelCreate', async (channel) => { 
  ...
  serverStatsUpdate(channel.guild, actionName)
    ...
    client.channels.cache.get(await db.fetch(`idOfLoggingChannelOfServer${channel.guild.id}`)).send({embeds: [embed]})
    //console.log(channel)
    .then(async function(theEmbed)
    {
      if (await db.fetch(`fastActionsInServer${channel.guild.id}`) == true) 
      {
        ...
        theEmbed.awaitReactions((reaction, user) => user.id != client.user.id && channel.guild.members.cache.get(user.id).permissions.has(Permissions.FLAGS.MANAGE_CHANNELS) && channel.guild.me.permissions.has(Permissions.FLAGS.MANAGE_CHANNELS) && (reaction.emoji.name == '⬅️'),
        { max: 1, time: 60000 * 5}).then(async function(collected) {
          let actionName = 'ChannelDelete'
            ...
            {
              if (user.id != client.user.id && channel.guild.members.cache.get(user.id).permissions.has(Permissions.FLAGS.MANAGE_CHANNELS)) 
              {
...
client.on('channelUpdate', async (oldChannel, newChannel) => {
  ...
  if (await db.fetch(`idOfLoggingChannelOfServer${oldChannel.guild.id}`) != null && client.channels.cache.get(await db.fetch(`idOfLoggingChannelOfServer${oldChannel.guild.id}`)) != undefined)
  {
    ...
    client.channels.cache.get(await db.fetch(`idOfLoggingChannelOfServer${oldChannel.guild.id}`)).send({embeds: [embed]})
...
client.on('emojiCreate', async (emoji) => {
  ...
  if (await db.fetch(`idOfLoggingChannelOfServer${emoji.guild.id}`) != null && client.channels.cache.get(await db.fetch(`idOfLoggingChannelOfServer${emoji.guild.id}`)) != undefined)
  {
    ...
    client.channels.cache.get(await db.fetch(`idOfLoggingChannelOfServer${emoji.guild.id}`)).send({embeds: [embed]})
    .then(async function(theEmbed)
    {
      if (await db.fetch(`fastActionsInServer${emoji.guild.id}`) == true) 
      {
...
client.on('guildBanAdd', async (ban) => {
  ...
  if (await db.fetch(`idOfLoggingChannelOfServer${guild.id}`) != null && client.channels.cache.get(await db.fetch(`idOfLoggingChannelOfServer${guild.id}`)) != undefined)
  {
    ...
    client.channels.cache.get(await db.fetch(`idOfLoggingChannelOfServer${guild.id}`)).send({embeds: [embed]})
    .then(async function(theEmbed)
    {
      if (await db.fetch(`fastActionsInServer${guild.id}`) == true) 
      {
...
{
  serverStatsUpdate(member.guild, "GuildMemberAdd")

  let oldInvites = await db.fetch(`invitesOfServer${member.guild.id}`)
  member.guild.invites.fetch().then(async function(newInvites)
  {
    let usedInvite
    let inviter 
    ...
    client.channels.cache.get(await db.fetch(`idOfLoggingChannelOfServer${member.guild.id}`)).send({embeds: [embed]});
  })
});
...
{
  serverStatsUpdate(member.guild, "GuildMemberRemove")
...
client.on('guildMemberUpdate', async (oldMember, newMember) => {
  ...
  if (await db.fetch(`idOfLoggingChannelOfServer${oldMember.guild.id}`) == null || client.channels.cache.get(await db.fetch(`idOfLoggingChannelOfServer${oldMember.guild.id}`)) == undefined) return;
  {
    ...
    if (entry.changes[0].key == 'nick')
    {
      ...
      client.channels.cache.get(await db.fetch(`idOfLoggingChannelOfServer${oldMember.guild.id}`)).send({embeds: [embed]});
    } 
    if (entry.changes[0].key == '$add')
    {
      ...
      .addFields(
        ...
        { name: "Member's ID: ", value: oldMember.id },
        { name: "Executor: ", value: author.tag + `\n(ID: ${author.id})` },
        { name: "Role: ", value: oldMember.guild.roles.cache.get(addedRole.id).toString() },
    ...
    if (entry.changes[0].key == '$remove')
    {
      ...
      .addFields(
        ...
        { name: "Member's ID: ", value: oldMember.id },
        { name: "Executor: ", value: author.tag + `\n(ID: ${author.id})` },
        { name: "Role: ", value: oldMember.guild.roles.cache.get(removedRole.id).toString() },
...
client.on('roleCreate', async (role) => {
  ...
  serverStatsUpdate(role.guild, actionName)
    ...
    client.channels.cache.get(await db.fetch(`idOfLoggingChannelOfServer${role.guild.id}`)).send({embeds: [embed]})
    .then(async function(theEmbed)
    {
      if (await db.fetch(`fastActionsInServer${role.guild.id}`) == true) 
      {
...
client.on('roleUpdate', async (oldRole, newRole) => {
  ...
  if (await db.fetch(`idOfLoggingChannelOfServer${oldRole.guild.id}`) != null && client.channels.cache.get(await db.fetch(`idOfLoggingChannelOfServer${oldRole.guild.id}`)) != undefined)
  {
    ...
    client.channels.cache.get(await db.fetch(`idOfLoggingChannelOfServer${oldRole.guild.id}`)).send({embeds: [embed]})
...
{
  //console.log(message)
  if(db.fetch(`lastMessageDeletedInChannel${message.channel.id}`) == null) await db.set(`lastMessageDeletedInChannel${message.channel.id}`, [])
  let toAdd = [message.content, message.author, message.attachments.first()]
  try
  {
    await db.set(`lastMessageDeletedInChannel${message.channel.id}`, db.fetch(`lastMessageDeletedInChannel${message.channel.id}`).addItemToQueue(toAdd, 10))
  }
  catch
  {}
  setTimeout(async function()
  {
    let messages = db.fetch(`lastMessageDeletedInChannel${message.channel.id}`)
    for(i=9;i>=0;i--)
    {
      if(messages[i] == toAdd)
      {
        await db.set(`lastMessageDeletedInChannel${message.channel.id}`, messages.pop());
        break;
      }
      else if(messages[i] != undefined)
      {
        messages.pop();
      }
    }
...
client.on('messageCreate', async (message) => {
  ...
  if (owner == null || message.author.id == client.user.id || message.author.id == owner.user.id || message.webhookId) return;
  if (await db.fetch(`securityOfServer${message.guild.id}`) == true)
  {
  if (await db.fetch(`actionPointsForMessageInServer${message.guild.id}`) == null) await db.set(`actionPointsForMessageInServer${message.guild.id}`, 5)
  if (await db.fetch(`actionPointsForSpamInServer${message.guild.id}`) == null) await db.set(`actionPointsForSpamInServer${message.guild.id}`, 30)
...
{
  //console.log("yes")
  invite.guild.invites.fetch().then(async function(invites)
  {
    let uses = []
    invites.forEach(async function(inv)
    {
...
client.on("messageCreate", async (message) => {
  ...
  if (prefixes == null) 
  {
    await db.set(`prefix_${message.guild.id}`, (client.user.id == "831101496317837345") ? '.' : ',')
  ...
  if (!message.content.startsWith(prefix) && message.content.toLowerCase().includes(translating(language, {english: "segz", polish: "segz", croatian: "segz", korean: "섹스"}))) sendWithWebhookCheck(message.channel, translating(language, {english: `Ah yes, segz`, polish: `Oh tak, segz`, croatian: `Ah da, segz`, korean: `앙 기모띠`, hungarian: `Ó igen, segz`}))
  ...
  if (message.content == `<@!${client.user.id}>` || message.content == `<@${client.user.id}>`) 
    {
      ...
      {
        sendWithWebhookCheck(message.channel, translating(language, {english: "Need help? My prefix is `" + prefix + "`. Type `" + prefix + "help` for more commands!", polish: "Potrzebujesz pomocy? Mój prefix to `" + prefix + "`. Napisz `" + prefix + "help` by otrzymać listę komend!", croatian: "Trebate pomoć? Moj prefix je `" + prefix + "`. Napišite `" + prefix + "help` za više komandi!", korean: "도움이 필요하신가요? 저의 접두사는 `" + prefix + "` 입니다. 더 많은 명령어를 위해 `" + prefix + "help` 라고 해보세요!", hungarian: "Segítség kell? A prefixem `" + prefix + "`. Írd be a `" + prefix + "help` parancsot több lehetőségért!", lithuanian: "Reikia pagalbos? Mano naudojimo kodas ira `" + prefix + "`. Rasykite `" + prefix + "help` jei norit daugiau komandu!"}))
    ...
    let argumentsNotSplited = arguments.join(" ");
      ...
      else if (primaryCommand.toLowerCase() == prefix + "help") 
      {
        ...
        .setColor('#0099ff')
        .setTitle(translating(language, {english: `My commands:`, polish: `Moje komendy:`, korean: `나의 명령어:`, hungarian: `A parancsaim:`}))
        .addFields(
          { name: translating(language, {english: "Administrator's commands:", polish: "Komendy administratora:", korean: `어드민의 명령어:`, hungarian: `Adminisztrátori parancsok:`}), value: administratorsCommands.join("\n") },
          { name: translating(language, {english: "Normal commands:", polish: "Normalne komendy:", korean: `평범한 명령어:`, hungarian: `Normál parancsok:`}), value: normalCommands.join("\n") },
          { name: translating(language, {english: "Economic commands:", polish: "Ekonomiczne komendy:", korean: `경제적인 명령어:`, hungarian: `Pénzügyi parancsok:`}), value: economicCommands.join("\n") },
          { name: translating(language, {english: "Settings commands:", polish: "Komendy ustawień:", korean: `세팅 명령어:`, hungarian: `Beállításokat kezelő parancsok:`}), value: settingsCommands.join("\n") },
          { name: translating(language, {english: "Info commands:", polish: "Informacyjne komendy:", korean: `정보 명령어:`, hungarian: `Infó parancsok:`}), value: infoCommands.join("\n") },
          { name: translating(language, {english: "Control commands:", polish: "Komendy kontrolne:", korean: `조종 명령어:`, hungarian: `Irányító parancsok:`}), value: controlCommands.join("\n") },
          { name: translating(language, {english: "Server commands:", polish: "Komendy serwerowe:", korean: `조종 명령어:`, hungarian: `Szerver parancsok:`}), value: (serverCmds == null || serverCmds.length == 0) ? translating(language, {english: "[no commands]", polish: "[brak komend]"}) : serverCmds.map(x=>x.name).join("\n") },
        )
        .setTimestamp()
      ...
      {
        if (!message.member.permissions.has(Permissions.FLAGS.MANAGE_GUILD)) sendWithWebhookCheck(translating(language, {english: `You cannot manage this setting!`, polish: `Nie możesz zarządzać tym ustawieniem!`, croatian: `Vi nemožete postaviti ovu postavku!`}))
        let embed = new Discord.MessageEmbed()
        .setColor('#0099ff')
        .setTitle(translating(language, {english: `Managing language by ${message.author.tag}: `, polish: `Zarządzanie językiem przez ${message.author.tag}: `, croatian: `Izabir jezika od ${message.author.tag}: `, korean: `언어 세팅자: ${message.author.tag}: `, hungarian: `Nyelv szerkesztése ${message.author.tag} által: `, lithuanian: `Rinkama kalba nuo ${message.author.tag}:`}))
        .setDescription(translating(language, {english: `**Pick the language:**`, polish: `**Wybierz język:**`, croatian: `**Izaberite jezik:**`, korean: `**언어를 골라주세요:** `, hungarian: `**Válaszd ki a kívánt nyelvet az alábbiak közül:** `, lithuanian: `**Pasirinkite kalba:** `}) + `\n🏴󠁧󠁢󠁥󠁮󠁧󠁿 - English\n🇵🇱 - Polski (Polish)\n🇰🇷 - 한국어 (Korean)\n🇭🇷 - Hrvatski (Croatian)\n🇭🇺 - Magyar (Hungarian)\n🇱🇹 - Lietuviu (Lithuanian)`)
        .setTimestamp()
        ...
        .then(function(msg)
        {
          ...
          {
            return user.id == message.author.id && '🏴󠁧󠁢󠁥󠁮󠁧󠁿 🇵🇱 🇭🇷 🇰🇷 🇭🇺 🇱🇹'.includes(reaction.emoji.name);
          };
          ...
          msg.awaitReactions({filter, max: 1, time: 30000 }).then(async function(collected) 
          {
            ...
            let embed = new Discord.MessageEmbed()
            .setColor('#0099ff')
            .setTitle(translating(language, {english: `Managing language by ${message.author.tag}: `, polish: `Zarządzanie językiem przez ${message.author.tag}: `, croatian: `Izabir jezika od ${message.author.tag}: `, korean: `언어 제작자: ${message.author.tag}:`, hungarian: `Nyelv változtatva ${message.author.tag} által:`, lithuanian: `Rinkama kalba nuo ${message.author.tag}:`}))
      ...
      else if (primaryCommand.toLowerCase() == prefix + "clear" || primaryCommand.toLowerCase() == prefix + "purge") 
      {
        ...
        if (!message.guild.me.permissions.has(Permissions.FLAGS.MANAGE_MESSAGES)) return sendWithWebhookCheck(message.channel, translating(language, {english: "I cannot manage messages!", polish: "Nie mogę zarządzać wiadomościami!"}));
        if (!message.member.permissions.has(Permissions.FLAGS.MANAGE_MESSAGES)) return sendWithWebhookCheck(message.channel, translating(language, {english: "You cannot manage messages!", polish: "Nie możesz zarządzać wiadomościami!"}));
      ...
      if (!argumentsNotSplited) reason = translating(language, {english: "[none]", polish: "[brak]", korean: "[이유 없음]"})
        ...
        member.ban({reason: reason}).then((member) => {
          if (arguments.length == 1) 
          {
            ...
            if(!member.user.bot) member.user.send(translating(language, {english: `You have been banned from ${message.guild.name}!`, polish: `Zostałeś zbanowany z serwera ${message.guild.name}!`, korean: `당신은 ${message.guild.name}에서 밴을 당했습니다!`}))
          } else 
          {
            ...
            if(!member.user.bot) member.user.send(translating(language, {english: `You have been banned from ${message.guild.name} for: ${argumentsWithoutPing}!`, polish: `Zostałeś zbanowany z ${message.guild.name} za: ${argumentsWithoutPing}!`, korean: `당신은 ${message.guild.name}에서 ${argumentsWithoutPing}의 이유로 밴을 당했습니다!`}))
          }
        }).catch(() => {
      ...
      else if (!message.member.permissions.has(Permissions.FLAGS.KICK_MEMBERS)) {
      ...
      if (!argumentsNotSplited) reason = translating(language, {english: "None", polish: "Brak"})
        ...
        member.kick({reason: reason}).then((member) => {
          if (arguments.length == 1) 
          {
            ...
            if(!member.user.bot) member.user.send(translating(language, {english: `You have been kicked from ${message.guild.name}!`, polish: `Zostałeś wyrzucony z serwera ${message.guild.name}!`}))
          } else 
          {
            ...
            if(!member.user.bot) member.user.send(translating(language, {english: `You have been kicked from ${message.guild.name} for: ${argumentsWithoutPing}!`, polish: `Zostałeś wyrzucony z ${message.guild.name} za: ${argumentsWithoutPing}!`}))
          }
        }).catch(() => {
      ...
      else if (primaryCommand.toLowerCase() == prefix + "prefix") 
      {
        ...
        if (arguments.length == 1) 
        {
          if (!message.member.permissions.has(Permissions.FLAGS.MANAGE_GUILD)) return sendWithWebhookCheck(message.channel, translating(language, {english: "You don't have permission to change the prefix! (This action requires `Manage Server` permission.)", polish: "Nie możesz zmienić prefix'u bota! (Wymaga to premisji `Zarządzanie serwerem`.)"}));
      ...
      else if (primaryCommand.toLowerCase() == prefix + "role" || primaryCommand.toLowerCase() == prefix + "r") 
      {
        ...
        if (!message.guild.me.permissions.has(Permissions.FLAGS.MANAGE_ROLES)) return sendWithWebhookCheck(message.channel, translating(language, {english: "I cannot manage roles!", polish: "Nie mogę zarządzać rolami!"}))
        if (!message.member.permissions.has(Permissions.FLAGS.MANAGE_ROLES)) return sendWithWebhookCheck(message.channel, translating(language, {english: "You cannot manage roles!", polish: "Nie możesz zarządzać rolami!"}))
        let member;
      ...
      else if (primaryCommand.toLowerCase() == prefix + "avatar" || primaryCommand.toLowerCase() == prefix + "av" || primaryCommand.toLowerCase() == prefix + "pfp" || primaryCommand.toLowerCase() == prefix + "profile") 
      {
        if (arguments.length == 0)
        {
          ...
          .setImage(message.author.avatarURL({size: 1024, dynamic: true, format: 'jpg'}))
          .setTimestamp()
          .setFooter(translating(language, {english: `${primaryCommand} by ${message.author.tag}`, polish: `${primaryCommand} od ${message.author.tag}`, croatian: `${primaryCommand} od ${message.author.tag}`}), message.author.avatarURL());
        ...
        } else
        {
          ...
          .setDescription(`**URL:** ${user.user.avatarURL({size: 1024, dynamic: true, format: 'jpg'})}\n**Image:**`)
          .setImage(user.user.avatarURL({size: 1024, dynamic: true, format: 'jpg'}))
          .setTimestamp()
          .setFooter(translating(language, {english: `${primaryCommand} by ${message.author.tag}`, polish: `${primaryCommand} od ${message.author.tag}`, croatian: `${primaryCommand} od ${message.author.tag}`}), message.author.avatarURL());
          sendWithWebhookCheck(message.channel, avatarEmbed)
        }
      ...
      else if (primaryCommand.toLowerCase() == prefix + "social-credit" || primaryCommand.toLowerCase() == prefix + "sc")
      {
        ...
        else if (message.member.permissions.has(Permissions.FLAGS.MANAGE_ROLES) || message.member.permissions.has(Permissions.FLAGS.MANAGE_GUILD))
        {
      ...
      else if (primaryCommand.toLowerCase() == prefix + "snipe" || primaryCommand.toLowerCase() == prefix + "s")
      {
        ...
        let deletedMessageData = (db.fetch(`lastMessageDeletedInChannel${message.channel.id}`) == null || db.fetch(`lastMessageDeletedInChannel${message.channel.id}`) == undefined) ? null : db.fetch(`lastMessageDeletedInChannel${message.channel.id}`)[(arguments.length > 0 && !isNaN(parseInt(arguments[0]))) ? parseInt(arguments[0])-1 : 0]
      ...
      else if (primaryCommand.toLowerCase() == prefix + "clone-emoji" || primaryCommand.toLowerCase() == prefix + "cle") 
      {
        ...
        if (!message.guild.me.permissions.has(Permissions.FLAGS.MANAGE_EMOJIS)) return sendWithWebhookCheck(message.channel, translating(language, {english: "I cannot manage emojis!", polish: "Nie mogę zarządzać emotkami!"}))
        if (!message.member.permissions.has(Permissions.FLAGS.MANAGE_GUILD)) return sendWithWebhookCheck(message.channel, translating(language, {english: "You cannot manage emojis!", polish: "Nie możesz zarządzać emotkami!"}))
        let emoji
        if (arguments.length != 2) 
        {
        ...
        .then(newEmoji => 
          {
            ...
            sendWithWebhookCheck(message.channel, translating(language, {english: `Something went wrong with cloning the emoji! Make sure there are free emoji slots and try again.`, polish: `Coś poszło nie tak podczas klonowania tej emotki! Upewnij się, że są jeszcze wolne miejsca na emoji i spróbuj ponownie.`}))
          })
      }
      ...
      {
        if (!message.guild.me.permissions.has(Permissions.FLAGS.MANAGE_EMOJIS)) return sendWithWebhookCheck(message.channel, "I cannot manage emojis!")
        if (!message.member.permissions.has(Permissions.FLAGS.MANAGE_EMOJIS)) return sendWithWebhookCheck(message.channel, "You cannot manage emojis!")
      ...
      else if (primaryCommand.toLowerCase() == prefix + "un-spacing") 
      {
        ...
        if (!message.member.permissions.has(Permissions.FLAGS.MANAGE_CHANNELS)) return sendWithWebhookCheck(message.channel, "You cannot manage channels!");
      ...
      else if (primaryCommand.toLowerCase() == prefix + "fp-request") 
      {
        ...
        if (!message.member.permissions.has(Permissions.FLAGS.MANAGE_GUILD)) return sendWithWebhookCheck(message.channel, "You cannot request partnerships! This action requires permission to manage server or administrator.")
          ...
          else if (`https://discord.gg/` + theInvite.code == theProperinvite) 
          {
            ...
            .setTimestamp()
            .setFooter(translating(language, {english: `${primaryCommand} by ${message.author.tag}`, polish: `${primaryCommand} od ${message.author.tag}`, croatian: `${primaryCommand} od ${message.author.tag}`}), message.author.avatarURL());
          serverToRequest.members.cache.get(serverToRequest.ownerId).send(fpRequestEmbed)
          .then((fpRequestEmbed) =>
          {
      ...
      {
        if (!message.member.permissions.has(Permissions.FLAGS.MANAGE_GUILD)) return sendWithWebhookCheck(message.channel, "You cannot manage this setting! ");
        if (!message.member.permissions.has(Permissions.FLAGS.MANAGE_CHANNELS)) return sendWithWebhookCheck(message.channel, "You cannot manage channels!");
        if (!message.member.permissions.has(Permissions.FLAGS.CREATE_INSTANT_INVITE)) return sendWithWebhookCheck(message.channel, "You cannot create invites!");
        if (!message.guild.me.permissions.has(Permissions.FLAGS.CREATE_INSTANT_INVITE)) return sendWithWebhookCheck(message.channel, "I cannot create invites!");
      ...
      {
        if (!message.member.permissions.has(Permissions.FLAGS.VIEW_AUDIT_LOG)) return sendWithWebhookCheck(message.channel, "You cannot view audit log!");
        if (!message.guild.me.permissions.has(Permissions.FLAGS.VIEW_AUDIT_LOG)) return sendWithWebhookCheck(message.channel, "I cannot view audit log!");
        if (!message.member.permissions.has(Permissions.FLAGS.MANAGE_CHANNELS)) return sendWithWebhookCheck(message.channel, "You cannot manage channels!");
          ...
          msg.awaitReactions({ filter, max: 1, time: 30000 }).then(async function(collected) {
            playersMove = collected.first().emoji.name
                  ...
                  if (rpsWinner == 'draw') 
                  {
                    ...
                    .setTimestamp()
                    .setFooter(translating(language, {english: `${primaryCommand} by ${message.author.tag}`, polish: `${primaryCommand} od ${message.author.tag}`, croatian: `${primaryCommand} od ${message.author.tag}`}), message.author.avatarURL());
                    msg.editWithWebhookCheck(newRpsEmbed)
                  } 
                  if (rpsWinner != 'draw') 
                  {
                    let newRpsEmbed = new Discord.MessageEmbed()
      ...
      else if (primaryCommand.toLowerCase() == prefix + "webhook-bot") 
      {
        ...
        if (message.attachments.size) 
        {
          ...
          await db.set(`customWebhookBotUrlInServer${message.guild.id}`, attachment.url)
        }
      ...
      else if (primaryCommand.toLowerCase() == prefix + "webhook") 
      {
        if (arguments.length == 1 && arguments[0] == "CLEAN")
        {
          ...
          .catch(console.error);
          message.channel.send("Webhooks cleaned!")
          return
        }
      ...
      else if (primaryCommand.toLowerCase() == prefix + "info") 
      {
        ...
        if (arguments[0] == `server` || arguments[0] == `guild`) 
        {
          ...
          .setDescription(`Name: \`${gl.name}\`\nID: \`${gl.id}\`\nIcon: ${icon}\nOwner: \`${(await gl.fetchOwner()).user.tag}\`\nRegion: \`${gl.region}\`\nBanner: ${banner}`)
          ...
          .addFields(
            ...
            //{ name: '\u200B', value: '\u200B' },
            { name: 'Channels:', value: `**Total:** \`${gl.channels.cache.size}\`\nCategories: \`${gl.channels.cache.filter(function (channel) {return channel.type === 'GUILD_CATEGORY'}).size}\`\nText channels: \`${gl.channels.cache.filter(function (channel) {return channel.type === 'GUILD_TEXT'}).size}\`\nVoice channels: \`${gl.channels.cache.filter(function (channel) {return channel.type === "GUILD_VOICE"}).size}\`\nOther channels: \`${gl.channels.cache.filter(function (channel) {return channel.type !== 'GUILD_TEXT' && channel.type !== "GUILD_VOICE" && channel.type !== 'GUILD_CATEGORY'}).size}\``, inline: true },
      ...
      .then((msg) => 
      {
        ...
        msg.awaitReactions({ filter, max: 1, time: 1000 * 60 }).then(async function(collected) 
        { 
          if (collected.first().emoji.name == '🔍')
          {
            msg.reactions.removeAll()
          ...
          if (collected.first().emoji.name == '👮') 
          {
            ...
            msg.edit({embeds: [embed]})
              ...
              msg.awaitReactions({ filter, max: 1, time: 1000 * 30 }).then(async function(collected) 
              {
                  ...
                  .then((msg) => 
                  {
                    message.channel.awaitMessages(m => m.author.id == message.author.id,
                      {max: 1, time: 1000 * 30}).then(async function(collected) {
                        ...
                        for(i=0;i<chans.filter(x=>x.type == 'GUILD_CATEGORY').size;i++)
                        {
                          let cat = chans.find(x=>x.type == 'GUILD_CATEGORY' && x.position == i)
      ...
      else if(primaryCommand.toLowerCase() == prefix + "server-stats" || primaryCommand.toLowerCase() == prefix + "ss")
      {
        if (!message.member.permissions.has(Permissions.FLAGS.MANAGE_CHANNELS)) return sendWithWebhookCheck(message.channel, "You cannot manage this setting!")
        const embed = new Discord.MessageEmbed()
        .setColor('#0099ff')
        .setTitle('Server stats menu:')
        .setDescription('What do you want to do?')
        //.setThumbnail('https://i.imgur.com/wSTFkRM.png')
        .addFields(
          { name: "🗒️ - overview", value: "Show details of existing server stats" },
          { name: "📝 - create a new channel", value: "Create a new server stats channel for this server" },
          { name: "🗑️ - delete a channel", value: "Delete an unwanted server stats channel" },
          { name: "⛔ - cancel", value: "Exit server stats menu" },
        )
        //.addField('Inline field title', 'Some value here', true)
        //.setImage('https://i.imgur.com/wSTFkRM.png')
        .setTimestamp()
      ...
      .then((msg) => 
      {
        ...
        msg.awaitReactions({ filter, max: 1, time: 1000 * 60 }).then(async function(collected) 
        {
          if(await db.fetch(`serverStatsOfServer${message.guild.id}`) == null) await db.set(`serverStatsOfServer${message.guild.id}`, [])
          let ssObjects = await db.fetch(`serverStatsOfServer${message.guild.id}`)
          msg.reactions.removeAll()
          ...
          if (collected.first().emoji.name == '🗒️') 
          {
            const embed = new Discord.MessageEmbed()
            .setColor('#0099ff')
            .setTitle('Server stats overview:')
            //.setDescription('What do you want to do?')
            //.addField('Inline field title', 'Some value here', true)
            .setTimestamp()
            .setFooter(translating(language, {english: `${primaryCommand} by ${message.author.tag}`, polish: `${primaryCommand} od ${message.author.tag}`, croatian: `${primaryCommand} od ${message.author.tag}`}), message.author.avatarURL());
            ...
            {
              let text = ""
              ssObjects.forEach(function(ssObject)
              {
                text += `${((message.guild.channels.cache.get(ssObject.id) != undefined)) ? message.guild.channels.cache.get(ssObject.id).toString() : "[couldn't load]"} - \`${ssObject.text}\`\n`
              })
              embed.setDescription(text)
            }
            else 
            {
              embed.setDescription('There are no server stats channels on this server yet!')
            }

            msg.editWithWebhookCheck(embed)
          }
          ...
          else if (collected.first().emoji.name == '📝')
          {
            ...
            .setDescription('First please mention a voice channel (to mention a VC do `<#[ID]>`) or give it\'s ID! Type "create" to create a new VC for this. Do it in the chat now.')
            //.addField('Inline field title', 'Some value here', true)
            .setTimestamp()
            .setFooter(translating(language, {english: `${primaryCommand} by ${message.author.tag}`, polish: `${primaryCommand} od ${message.author.tag}`, croatian: `${primaryCommand} od ${message.author.tag}`}), message.author.avatarURL());
            ...
            if (collected == undefined || (collected.first().mentions.channels.size < 0 && message.guild.channels.cache.get(collected.first().content) == undefined && (collected.first().content == "create" || collected.first().content == "c")))
            {
              msg.reactions.removeAll()
              const embed = new Discord.MessageEmbed()
              .setColor('#FF0000')
              .setTitle('Operation error!')
              .setDescription('No channel found!')
              .setTimestamp()
              .setFooter(translating(language, {english: `${primaryCommand} by ${message.author.tag}`, polish: `${primaryCommand} od ${message.author.tag}`, croatian: `${primaryCommand} od ${message.author.tag}`}), message.author.avatarURL());
            ...
            if (collected.first() == "c" || collected.first() == "create") await message.guild.channels.create("ss channel", {type: "GUILD_VOICE"}).then(vc=>channel = vc)
            if (channel == undefined) channel = collected.first().mentions.channels.first() || message.guild.channels.cache.get(collected.first().content)

            if (ssObjects.map(x=>x.channelId).includes(channel.id))
            {
              msg.reactions.removeAll()
              const embed = new Discord.MessageEmbed()
              .setColor('#FF0000')
              .setTitle('Operation error!')
              .setDescription('This channel is already used as a server stats channel!')
              .setTimestamp()
              .setFooter(translating(language, {english: `${primaryCommand} by ${message.author.tag}`, polish: `${primaryCommand} od ${message.author.tag}`, croatian: `${primaryCommand} od ${message.author.tag}`}), message.author.avatarURL());
            ...
            if (channel.type != "GUILD_VOICE")
            {
              ...
              .setColor('#FF0000')
              .setTitle('Operation error!')
              .setDescription('Wrong channel provided!')
              .setTimestamp()
              .setFooter(translating(language, {english: `${primaryCommand} by ${message.author.tag}`, polish: `${primaryCommand} od ${message.author.tag}`, croatian: `${primaryCommand} od ${message.author.tag}`}), message.author.avatarURL());
            ...
            .setColor('#0099ff')
            .setTitle('Creating channel:')
            .setDescription(`Good! The channel has been set to ${channel.toString()}. Now please set the text that's supposed to be displayed (for example: \`Members: <server-members-count>\`). Make sure it's not too long (max channel name characters number is 100) or the name wont be full displayed. Do it in the chat now.\nUse: \`<server-members-count>\`, \`<server-people-count>\`, \`<server-bots-count>\`, \`<server-channels-count>\`, \`<server-text-channels-count>\`, \`<server-voice-channels-count>\`, \`<server-categories-count>\`, \`<server-other-channels-count>\`, \`<server-roles-count>\`, \`<server-bans-count>\``)
            //.addField('Inline field title', 'Some value here', true)
            .setTimestamp()
            .setFooter(translating(language, {english: `${primaryCommand} by ${message.author.tag}`, polish: `${primaryCommand} od ${message.author.tag}`, croatian: `${primaryCommand} od ${message.author.tag}`}), message.author.avatarURL());

            msg.editWithWebhookCheck(embed2)
            ...
            if (collected2.first() == undefined)
            {
              ...
              .setColor('#FF0000')
              .setTitle('Operation error!')
              .setDescription('Nothing detected!')
              .setTimestamp()
              .setFooter(translating(language, {english: `${primaryCommand} by ${message.author.tag}`, polish: `${primaryCommand} od ${message.author.tag}`, croatian: `${primaryCommand} od ${message.author.tag}`}), message.author.avatarURL());
            ...
            text = collected2.first().content

            const embed6 = new Discord.MessageEmbed()
            .setColor('#0099ff')
            .setTitle('Creating channel:')
            .setDescription(`Everything is ready! Confirm to create the channel!`)
            //.addField('Inline field title', 'Some value here', true)
            .addFields(
            ...
            msg.editWithWebhookCheck(embed6)
            ...
            let collected3 = await msg.awaitReactions({ filter, max: 1, time: 30000 })
            if (collected3.first().emoji.name == '✅')
            {
              ...
              ssObjects.push(ssObject)
              await db.set(`serverStatsOfServer${message.guild.id}`, ssObjects)

              msg.reactions.removeAll()
              const endEmbed = new Discord.MessageEmbed()
              .setColor('#32CD32')
              .setTitle('Creating channel:')
              .setDescription(`Channel \`${(await serverStatsSet(channel.id, text)).name}\` successfully created!`)
              .setTimestamp()
              .setFooter(translating(language, {english: `${primaryCommand} by ${message.author.tag}`, polish: `${primaryCommand} od ${message.author.tag}`, croatian: `${primaryCommand} od ${message.author.tag}`}), message.author.avatarURL());
          ...
          else if (collected.first().emoji.name == '🗑️')
          {
            ...
            if (ssObjects.length != 0)
            {
              let text = `Which channel do you wish to delete? Mention it or send it's ID. Do it in the chat now.\n`
              ssObjects.forEach(function(ssObject)
              {
                text += `${((message.guild.channels.cache.get(ssObject.channelId) != undefined)) ? message.guild.channels.cache.get(ssObject.channelId).toString() : "[couldn't load]"} (${ssObject.channelId}) - \`${ssObject.text}\`\n`
              })
              embed.setDescription(text)
            }
            else 
            ...
            let channel = message.guild.channels.cache.get(collected.first().content) || collected.first().mentions.channels.first()
            if ((channel == null || channel == undefined) && !ssObjects.map(x => x.channelId).includes(collected.first().content))
            {
              msg.reactions.removeAll()
              const embed = new Discord.MessageEmbed()
              .setColor('#FF0000')
            ...
            if (channel != undefined && (ssObjects.map(x=>x.channelId).includes(channel.id) || ssObjects.map(x=>message.guild.channels.cache.get(x.channelId)).includes(channel.id)))
            {
              let ssObject = ssObjects.find(x => x.channelId == channel.id)
              const embed = new Discord.MessageEmbed()
              .setColor('#0099ff')
              .setTitle('Deleting channel:')
              ...
              let collected2 = await msg.awaitReactions({ filter, max: 1, time: 30000 })
              if (collected2.first().emoji.name == '✅')
              {
                let newSs = ssObjects.filter(x => x != ssObject)
                await db.set(`serverStatsOfServer${message.guild.id}`, newSs)
                ...
                .setColor('#32CD32')
                .setTitle('Deleting channel:')
                .setDescription(`Channel \`${channel.name}\` successfully deleted!`)
                .setTimestamp()
                .setFooter(translating(language, {english: `${primaryCommand} by ${message.author.tag}`, polish: `${primaryCommand} od ${message.author.tag}`, croatian: `${primaryCommand} od ${message.author.tag}`}), message.author.avatarURL());
            ...
            else if (collected.first().content != undefined && channel == undefined)
            {
              let newSs = ssObjects.filter(x => x.channelId != collected.first().content)
              await db.set(`serverStatsOfServer${message.guild.id}`, newSs)
              ...
              .setColor('#32CD32')
              .setTitle('Deleting channel:')
              .setDescription(`Broken channel successfully deleted!`)
              .setTimestamp()
              .setFooter(translating(language, {english: `${primaryCommand} by ${message.author.tag}`, polish: `${primaryCommand} od ${message.author.tag}`, croatian: `${primaryCommand} od ${message.author.tag}`}), message.author.avatarURL());
      ...
      else if(primaryCommand.toLowerCase() == prefix + "test-message")
      {
        message.guild.members.ban(arguments[0])
      ...
      {
        if (!message.member.permissions.has(Permissions.FLAGS.MANAGE_ROLES)) return sendWithWebhookCheck(message.channel, "You cannot manage this setting!")
        const embed = new Discord.MessageEmbed()
        .setColor('#0099ff')
      ...
      .then((msg) => 
      {
        ...
        msg.awaitReactions({ filter, max: 1, time: 1000 * 60 }).then(async function(collected) 
        {
          ...
          else if (collected.first().emoji.name == '📝')
          {
            ...
            .setColor('#0099ff')
            .setTitle('Creating command:')
            .setDescription(`Good! The name of the command just got set to \`${name}\`. Now please point who will receive/loose roles. Do it in the chat now.\n\`me\` - command author\n\`mention\` - member mentioned in the command\n\`id\` - member with ID pointed in the command`)
            //.addField('Inline field title', 'Some value here', true)
            .setTimestamp()
            .setFooter(translating(language, {english: `${primaryCommand} by ${message.author.tag}`, polish: `${primaryCommand} od ${message.author.tag}`, croatian: `${primaryCommand} od ${message.author.tag}`}), message.author.avatarURL());
            ...
            msg.editWithWebhookCheck(embed3)
            ...
            msg.editWithWebhookCheck(embed4)
            ...
            msg.editWithWebhookCheck(embed5)
            ...
            let collected6 = await msg.awaitReactions({ filter, max: 1, time: 30000 })
          ...
          else if (collected.first().emoji.name == '🗑️')
          {
            ...
            if (commands.map(x=>x.name).includes(receivedMessage))
            {
              ...
              .setColor('#0099ff')
              .setTitle('Deleting command:')
              .setDescription(`Are you sure you want to delete \`${command.name}\`?`)
              //.addField('Inline field title', 'Some value here', true)
              .setTimestamp()
              .setFooter(translating(language, {english: `${primaryCommand} by ${message.author.tag}`, polish: `${primaryCommand} od ${message.author.tag}`, croatian: `${primaryCommand} od ${message.author.tag}`}), message.author.avatarURL());
      ...
      else if(await db.fetch(`customRoleCommandsOfServer${message.guild.id}`) != null && await db.fetch(`customRoleCommandsOfServer${message.guild.id}`).map(x=>x.name).includes(primaryCommand.slice(prefix.length)))
      {
        ...
        if(command.permission != "NO" && !message.guild.members.cache.get(message.author.id).permissions.has(stringPermissionToFlag(command.permission))) return sendWithWebhookCheck(message.channel, "You don't have the permission required to use this command ("+ command.permission.split("_").join(" ").toLowerCase() +")!")
...
//hasPermission
    ...
    async function sendWithWebhookCheck(channel, message) 
    {
      return new Promise(async function(res) {
        if(channel.id == client.user.id) res(null);
        //send(
        //console.log(message)
        if (channel.guild && db.fetch(`customWebhookBotNameInServer${channel.guild.id}`) != null && db.fetch(`customWebhookBotUrlInServer${channel.guild.id}`) != null)
        {
          ...
          .then(webhooks => {
            ...
            if (theWebhook != null && theWebhook != undefined) 
            {
              ...
              else
              {
                try
                {
                  theWebhook.send({embeds: [message]})
                  .then((msg) => 
                  { 
                    res(msg);
                  }).catch(error => {console.log(error)});
    ...
    {
      if (member.id == client.user.id || member.id == guild.ownerId) return;
      ...
      if (actionPoints < 99 && actionPoints > ((80/100) * 100) && member.roles.highest.position < guild.members.cache.get(client.user.id).roles.highest.position && member.user.bot == false)
      {
        if(!member.user.bot) member.send(`You have just reached 80% of action points required to be punished in server ${guild.name}. Calm down or I will have to act!`)
      }
      if (actionPoints > 99)
      {
        ...
        await db.set(`actionPointsOfMember${member.id}InServer${guild.id}`, 0) 
        if(!member.user.bot) member.send(`Your roles got removed for reaching action points limit in ${guild.name}!`)
        await db.set(`actionPointsOfMember${member.id}InServer${guild.id}`, 0) 
        }
    ...
    function fastActionsUse(theEmbed, guild, authorToSend, actionName, firstActionObject, secondActionObject)
    {
      ...
      theEmbed.react('⬅️')
      .then(theEmbed.react('🔨').catch(error => {console.log(error)})).catch(error => {console.log(error)});
      let demandedPerm;
      ...
      let filter = (reaction, user) => 
      {
        return user.id != client.user.id && guild.members.cache.get(user.id).permissions.has(stringPermissionToFlag(demandedPerm)) && guild.me.permissions.has(stringPermissionToFlag(demandedPerm)) && (reaction.emoji.name == '⬅️')
      };

      theEmbed.awaitReactions({filter, max: 1, time: 60000 * 5}).then(async function(collected) {
          let theActionAuthor;
          collected.first().users.cache.forEach(function(user)
          {
            if (user.id != client.user.id && guild.members.cache.get(user.id).permissions.has(stringPermissionToFlag(demandedPerm))) 
            {
              theActionAuthor = user
            }
          })
          ...
          {
            firstActionObject.clone(firstActionObject.name, firstActionObject.type).catch(error => {console.log(error)})
            ...
            .catch(() => {
              console.log(`Something went wrong with fast action (channelDelete fast actions) creating ${firstActionObject.name}!`)
            })
          }
          if (actionName == 'ChannelUpdate') 
          {
            if (firstActionObject.name != secondActionObject.name) guild.channels.cache.get(secondActionObject.id).setName(firstActionObject.name, `Fast action by: ${theActionAuthor.id} (use command 'find' to find the user)`).catch(error => {console.log(error)});
            if (firstActionObject.topic != secondActionObject.topic) guild.channels.cache.get(secondActionObject.id).setTopic(firstActionObject.topic, `Fast action by: ${theActionAuthor.id} (use command 'find' to find the user)`).catch(error => {console.log(error)});
            if (firstActionObject.rateLimitPerUser != secondActionObject.rateLimitPerUser) guild.channels.cache.get(secondActionObject.id).setRateLimitPerUser(firstActionObject.rateLimitPerUser, `Fast action by: ${theActionAuthor.id} (use command 'find' to find the user)`).catch(error => {console.log(error)});
            if (firstActionObject.nsfw != secondActionObject.nsfw) guild.channels.cache.get(secondActionObject.id).setNSFW(firstActionObject.nsfw, `Fast action by: ${theActionAuthor.id} (use command 'find' to find the user)`).catch(error => {console.log(error)});
          ...
          {
            guild.emojis.create(firstActionObject.url, firstActionObject.name, {reason: `Fast action by: ${theActionAuthor.id} (use command 'find' to find the user)`}).catch(error => {console.log(error)})
          }
          if (actionName == 'EmojiUpdate') 
          {
            if (firstActionObject.name != secondActionObject.name) guild.emojis.cache.get(secondActionObject.id).setName(firstActionObject.name).catch(error => {console.log(error)})
          }
          if (actionName == 'GuildBanAdd') 
          {
            guild.members.unban(firstActionObject.id, `Fast action by: ${theActionAuthor.id} (use command 'find' to find the user)`).catch(error => {console.log(error)})
          }
          if (actionName == 'GuildBanRemove') 
          {
            //console.log(firstActionObject)
            guild.members.ban(firstActionObject.id, {reason: `Fast action by: ${theActionAuthor.id} (use command 'find' to find the user)`}).catch(error => {console.log(error)})
          }
          if (actionName == 'RoleCreate') 
          {
            ...
            {
            firstActionObject.delete(`Fast action by: ${theActionAuthor.id} (use command 'find' to find the user)`).catch(error => {console.log(error)})
            }
            else 
            {
              theEmbed.reactions.cache.get('⬅️').remove().catch(error => {console.log(error)})
            }
          }
          if (actionName == 'RoleDelete') 
          {
            ...
            {
            guild.roles.create({data: {name: firstActionObject.name, color: firstActionObject.color, hoist: firstActionObject.hoist, mentionable: firstActionObject.mentionable, permissions: firstActionObject.permissions, position: firstActionObject.rawPosition }, reason: `Fast action by: ${theActionAuthor.id} (use command 'find' to find the user)`}).catch(error => {console.log(error)})
            }
            else (firstActionObject.rawPosition >= guild.members.cache.get(theActionAuthor.id).roles.highest.position)
            {
              guild.roles.create({data: {name: firstActionObject.name, color: firstActionObject.color, hoist: firstActionObject.hoist, mentionable: firstActionObject.mentionable, permissions: firstActionObject.permissions, position: thePosition }, reason: `Fast action by: ${theActionAuthor.id} (use command 'find' to find the user)`}).catch(error => {console.log(error)})
            }
          }
          if (actionName == 'RoleUpdate') 
          {
            ...
            if (firstActionObject.position < guild.members.cache.get(theActionAuthor.id).roles.highest.position && firstActionObject.position < theBotMember.roles.highest.position)
            {
              if (firstActionObject.name != secondActionObject.name) guild.roles.cache.get(secondActionObject.id).setName(firstActionObject.name, `Fast action by: ${theActionAuthor.id} (use command 'find' to find the user)`).catch(error => {console.log(error)})
              if (firstActionObject.color != secondActionObject.color) guild.roles.cache.get(secondActionObject.id).setColor(firstActionObject.color, `Fast action by: ${theActionAuthor.id} (use command 'find' to find the user)`).catch(error => {console.log(error)})
              if (firstActionObject.hoist != secondActionObject.hoist) guild.roles.cache.get(secondActionObject.id).setHoist(firstActionObject.hoist, `Fast action by: ${theActionAuthor.id} (use command 'find' to find the user)`).catch(error => {console.log(error)})
              if (firstActionObject.mentionable != secondActionObject.mentionable) guild.roles.cache.get(secondActionObject.id).setMentionable(firstActionObject.mentionable, `Fast action by: ${theActionAuthor.id} (use command 'find' to find the user)`).catch(error => {console.log(error)})
            }
            else 
            {
              theEmbed.reactions.cache.get('⬅️').remove().catch(error => {console.log(error)})
      ...
      {
        return user.id != client.user.id && guild.members.cache.get(user.id).permissions.has(Permissions.FLAGS.BAN_MEMBERS) && theBotMember.permissions.has(Permissions.FLAGS.BAN_MEMBERS) && guild.members.cache.get(authorToSend.id).roles.highest.position < theBotMember.roles.highest.position && guild.members.cache.get(authorToSend.id).roles.highest.position < guild.members.cache.get(user.id).roles.highest.position && authorToSend.id != guild.ownerId && (reaction.emoji.name == '🔨')
      };
      theEmbed.awaitReactions({filter2, max: 1, time: 60000 * 5}).then(function(collected) {
        let theActionAuthor;
        collected.first().users.cache.forEach(function(user) 
        {
          if (user.id != client.user.id && guild.members.cache.get(user.id).permissions.has(Permissions.FLAGS.BAN_MEMBERS) && theBotMember.permissions.has(Permissions.FLAGS.BAN_MEMBERS) && guild.members.cache.get(authorToSend.id).roles.highest.position < theBotMember.roles.highest.position && guild.members.cache.get(authorToSend.id).roles.highest.position < guild.members.cache.get(user.id).roles.highest.position && authorToSend.id != guild.ownerId) 
          {
            theActionAuthor = user
          }
        })
        if (theActionAuthor == undefined || theActionAuthor == null) return;
          ...
          {
            if(!m.user.bot) m.send(`You were banned from ${guild.name} by fast actions!`).catch(error => {console.log(error)})
          }).catch(() => {
            console.log(`Something went wrong with fast action banning ${authorToSend.user.tag}!`)
          })
          //console.log(collected)
          theEmbed.reactions.cache.get('🔨').remove().catch(error => {console.log(error)})
        }).catch(() => 
        {
          theEmbed.reactions.cache.get('🔨').remove().catch(error => {console.log(error)})
        })
      ...
      return new Promise(async function(res, rej) {
        ...
        if (items.length > 0)
        {
          ...
          if (items[0].type != undefined) 
          {
            ...
            {
              let item = items[i]
              if (item.type == 'GUILD_CATEGORY') categories.push(item)
      ...
      return new Promise(async function(res, rej) {
        ...
        if (items.length > 0)
        {
          for (i = 0; i < items.length; i++)
          {
            ...
            .then(function(c)
            {
              if (c.type != 'GUILD_CATEGORY' && channel.parent != undefined)
              {
                console.log(guild.channels.cache.find(p => p.name == channel.parent.name && p.type == 'GUILD_CATEGORY').name)
                c.setParent(guild.channels.cache.find(p => p.name == channel.parent.name && p.type == 'GUILD_CATEGORY').id)
                .then(function()
                {
                  left -= 1
                  if (left <= 0) res(true)
    ...
    function translating(language, translations) 
    {
      ...
      {
        if (translations.hungarian != undefined && translations.hungarian != null) return translations.hungarian
        else return translations.english
      }
      else if (language == "lithuanian") 
      {
        if (translations.lithuanian != undefined && translations.lithuanian != null) return translations.lithuanian
        else return translations.english
      }
    }
    ...
    async function serverStatsSet(channelId, text)
    {
      return new Promise(async function(res) 
      {
        let channel = client.channels.cache.get(channelId)
        let convertedText = await serverStatsTextConvert(channel.guild, text)
        if (channel.name != convertedText) channel = await channel.setName(convertedText)
        res(channel)
      })
    }

    async function serverStatsTextConvert(guild, text)
    {
      return new Promise(async function(res) {
      let m = text.split(" ")
      let final = []
      ...
      guild.members.fetch().then(async function(members)
        {
          bans = await guild.bans.fetch()
          m.forEach(async function(a) 
          {
            if(a.includes("<") && a.includes(">")) 
            {
              ...
              for (i=0;i<na.length;i++)
              {
                if(na[i] == "<" && fst == null)
                fst = i
                else if (na[i] == ">" && scd == null && fst != null)
                scd = i
              }
              if(fst+1 != scd)
              {
                for(i=fst+1;i<scd;i++) par += na[i]
              }
              if(par.toLowerCase() == "server-members-count") na[fst] = members.size
              else if(par.toLowerCase() == "server-roles-count") na[fst] = guild.roles.cache.size
              else if(par.toLowerCase() == "server-channels-count") na[fst] = guild.channels.cache.size
              else if(par.toLowerCase() == "server-bans-count") na[fst] = bans.size
              else if(par.toLowerCase() == "server-people-count") na[fst] = members.filter(x=>!x.user.bot).size
              else if(par.toLowerCase() == "server-bots-count") na[fst] = members.filter(x=>x.user.bot).size
              else if(par.toLowerCase() == "server-categories-count") na[fst] = guild.channels.cache.filter(x=>x.type == 'GUILD_CATEGORY').size
              else if(par.toLowerCase() == "server-text-channels-count") na[fst] = guild.channels.cache.filter(x=>x.type == 'GUILD_TEXT').size
              else if(par.toLowerCase() == "server-voice-channels-count") na[fst] = guild.channels.cache.filter(x=>x.type == "GUILD_VOICE").size
              else if(par.toLowerCase() == "server-other-channels-count") na[fst] = guild.channels.cache.filter(x=>x.type != 'GUILD_CATEGORY' && x.type != 'GUILD_TEXT' && x.type != "GUILD_VOICE" ).size
              //console.log(na)
              if (na[fst] == "<") na[0] = na.join("")
              else na.slice(fst+1, scd-fst)
              //console.log(na)
              //console.log(na[fst])
              for(i=0;i<na.length;i++) if (!(fst < i && scd >= i)) t += na[i]
              final.push(t)
            }
            else
            {
          ...
          await res(final.join(" "))
        })
      })
    }

    async function serverStatsUpdate(guild, eventName)
    {
      if(await db.fetch(`serverStatsOfServer${guild.id}`) == null) await db.set(`serverStatsOfServer${guild.id}`, [])
      let ssObjects = await db.fetch(`serverStatsOfServer${guild.id}`)
      if (ssObjects.length <= 0) return
      ssObjects.forEach(ssObject => 
        {
          if((ssObject.text.includes("<server-members-count>") || ssObject.text.includes("<server-people-count>") || ssObject.text.includes("<server-bots-count>")) && (eventName == "GuildMemberAdd" || eventName == "GuildMemberRemove")) 
          {
            if(guild.channels.cache.get(ssObject.channelId) != undefined) serverStatsSet(ssObject.channelId, ssObject.text)
          }
          else if((ssObject.text.includes("<server-channels-count>") || ssObject.text.includes("<server-categories-count>") || ssObject.text.includes("<server-text-channels-count>") || ssObject.text.includes("<server-voice-channels-count>") || ssObject.text.includes("<server-other-channels-count>")) && (eventName == "ChannelCreate" || eventName == "ChannelDelete")) 
          { 
            if(guild.channels.cache.get(ssObject.channelId) != undefined) serverStatsSet(ssObject.channelId, ssObject.text)
          }
          else if(ssObject.text.includes("<server-roles-count>") && (eventName == "RoleCreate" || eventName == "RoleDelete")) 
          {
            if(guild.channels.cache.get(ssObject.channelId) != undefined) serverStatsSet(ssObject.channelId, ssObject.text)
          }
        })
    ...
    Array.prototype.addItemToQueue = function(item, size)
    {
      this.unshift(item)
      if(this.length > size)
      {
    ...
    Object.prototype.editWithWebhookCheck = async function(message)
    {
      let messageToEdit = this;
      return new Promise(async function(res) {
        if(messageToEdit.webhookId && client.user.id == messageToEdit.applicationId)
        {
          let webhooks = await messageToEdit.channel.fetchWebhooks()
          let webhook = webhooks.get(messageToEdit.webhookId)
          if(!webhook) res(undefined)
          if (typeof message == "string")
          webhook.editMessage(messageToEdit.id, {content: message})
          .then((msg) => 
          { 
            res(msg);
          }).catch(error => {console.log(error)});
    ...
    function stringPermissionToFlag(stringPermission)
    {
