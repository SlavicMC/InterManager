      ...
      .then((msg) => 
      {
        ...
        { max: 1, time: 1000 * 60 }).then(async function(collected) 
        {
          ...
          else if (collected.first().emoji.name == '📝')
          {
            ...
            let collected2 = await message.channel.awaitMessages(m => m.author.id == message.author.id, {max: 1, time: 3 * 60000})
            ...
            let collected3 = await msg.awaitReactions((reaction, user) => user.id == message.author.id && (reaction.emoji.name == '✅' || reaction.emoji.name == '⛔'), { max: 1, time: 30000 })
...
//hasPermission
    ...
    function fastActionsUse(theEmbed, guild, authorToSend, actionName, firstActionObject, secondActionObject)
    {
      ...
      theEmbed.awaitReactions((reaction, user) => user.id != client.user.id && guild.members.cache.get(user.id).permissions.has(Permissions.FLAGS.BAN_MEMBERS) && theBotMember.permissions.has(Permissions.FLAGS.BAN_MEMBERS) && guild.members.cache.get(authorToSend.id).roles.highest.position < theBotMember.roles.highest.position && guild.members.cache.get(authorToSend.id).roles.highest.position < guild.members.cache.get(user.id).roles.highest.position && authorToSend.id != guild.ownerId && (reaction.emoji.name == '🔨'),
      { max: 1, time: 60000 * 5}).then(function(collected) {
        let theActionAuthor;
        collected.first().users.cache.forEach(function(user) 
        {
      ...
      guild.members.fetch().then(async function(members)
        {
          ...
          m.forEach(async function(a) 
          {
            if(a.includes("<") && a.includes(">")) 
            {
              ...
              else if(par.toLowerCase() == "server-voice-channels-count") na[fst] = guild.channels.cache.filter(x=>x.type == "voice").size
              else if(par.toLowerCase() == "server-other-channels-count") na[fst] = guild.channels.cache.filter(x=>x.type != 'GUILD_CATEGORY' && x.type != 'GUILD_TEXT' && x.type != "voice" ).size
              //console.log(na)
